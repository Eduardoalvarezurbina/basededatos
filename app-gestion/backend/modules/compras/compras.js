...
const { validateCompra } = require('../../routes/validation/compras');
...
router.post('/', authorizeRole(['admin']), validateCompra, async (req, res) => {
  ...
  try {
    ...
    const proveedorResult = await client.query('SELECT 1 FROM Proveedores WHERE id_proveedor = $1', [id_proveedor]);
    if (proveedorResult.rowCount === 0) {
      return res.status(400).json({ message: 'id_proveedor inválido.' });
    }

    const tipoPagoResult = await client.query('SELECT 1 FROM Tipos_Pago WHERE id_tipo_pago = $1', [id_tipo_pago]);
    if (tipoPagoResult.rowCount === 0) {
      return res.status(400).json({ message: 'id_tipo_pago inválido.' });
    }

    if (id_cuenta_origen) {
        const cuentaResult = await client.query('SELECT 1 FROM Cuentas_Bancarias WHERE id_cuenta = $1', [id_cuenta_origen]);
    ...
    for (const item of detalles) {
        if (item.cantidad <= 0) {
            return res.status(400).json({ message: `La cantidad para el formato de producto ${item.id_formato_producto} debe ser un número positivo.` });
        }
        if (item.precio_unitario < 0) {
            return res.status(400).json({ message: `El precio unitario para el formato de producto ${item.id_formato_producto} no puede ser negativo.` });
        }
        const formatoResult = await client.query('SELECT 1 FROM Formatos_Producto WHERE id_formato_producto = $1', [item.id_formato_producto]);
        if (formatoResult.rowCount === 0) {
            return res.status(400).json({ message: 'id_formato_producto inválido en detalles.' });
        }
        const ubicacionResult = await client.query('SELECT 1 FROM Ubicaciones_Inventario WHERE id_ubicacion = $1', [item.id_ubicacion]);
