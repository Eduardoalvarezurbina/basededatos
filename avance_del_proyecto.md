...
- [x] Implementación del módulo de gestión de pedidos (CRUD).
  - **Implementación de Creación de Pedidos:** Se añadió el endpoint `POST /api/pedidos` para la creación de nuevos pedidos. Este endpoint permite registrar un pedido con un cliente, fecha de agendamiento, lugar y tipo de entrega, observación y una lista de productos detallados. La operación se realiza dentro de una transacción para asegurar la integridad de los datos.
