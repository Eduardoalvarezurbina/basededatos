...
- [x] Implementación del módulo de gestión de pedidos (CRUD).
  - **Implementación del CRUD completo de Pedidos:** Se implementó el CRUD completo para la gestión de pedidos, incluyendo los endpoints `POST /api/pedidos`, `GET /api/pedidos`, `GET /api/pedidos/:id`, `PUT /api/pedidos/:id` y `DELETE /api/pedidos/:id`. Esto permite la creación, lectura, actualización (de estado y observación) y eliminación de pedidos, junto con sus detalles. Todas las operaciones se realizan dentro de una transacción para asegurar la integridad de los datos.
- [x] Implementación del módulo de gestión de inventario (CRUD).
  ...
  - **Implementación de Transformaciones Complejas:** Se modificó la función `transformarProducto` en `produccionController.js` para soportar procesos con múltiples ingredientes y múltiples productos de salida, utilizando las tablas `Detalle_Procesos` y `Detalle_Procesos_Salida`. Se crearon las migraciones DDL `042_alter_procesos_for_multiple_outputs.sql` (para crear la tabla `Detalle_Procesos_Salida` y eliminar `id_formato_producto_final` de `Procesos`) y `043_add_complejo_to_procesos_tipo_proceso.sql` (para añadir 'COMPLEJO' como un tipo de proceso válido). Se actualizaron las pruebas de integración para verificar esta nueva funcionalidad.
